...
// ===================== ALL MEALS (for search) =====================
const ALL_MEALS = [
  { id: "m1", name: "Jumbo Rice Combo",       vendor: "Pearl's Cuisine",      price: 3500,  emoji: "🍱", sponsored: true,  category: "combos" },
  { id: "m2", name: "Jollof Rice + Chicken",  vendor: "Mama Nkechi's",        price: 1800,  emoji: "🍚", sponsored: false, category: "rice"   },
    ...
    <>
      ...
      <div className="popup-overlay" onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div className="popup-card" onClick={e => e.stopPropagation()} style={{ background: "#0a0a0a", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 28, padding: "36px 32px", width: "100%", maxWidth: 400, textAlign: "center", boxShadow: "0 32px 80px rgba(0,0,0,0.6)" }}>
          ...
          <p style={{ color: "#6b7280", fontFamily: "'DM Sans', sans-serif", fontSize: 14, lineHeight: 1.7, marginBottom: 28 }}>You need a Kravely account to place an order. Free and takes less than a minute!</p>
                ...
                {zoneItems.map(loc => (
                  <div key={loc.id} className="loc-item"
                    ...
                    style={{
                      ...
                      background: selected?.id === loc.id ? "rgba(34,197,94,0.06)" : "transparent",
                      borderLeft: selected?.id === loc.id ? "2px solid #22c55e" : "2px solid transparent",
                    }}
                  >
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: zStyle.bg, border: `1px solid ${zStyle.border}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill={zStyle.color} stroke="none">
...
// ===================== NAVBAR =====================
function AppNavbar({ cartCount, onCartClick, onLoginClick, location, onLocationClick, searchQuery, onSearchChange }) {
  ...
  return (
    <>
      <style>{`
        ...
        @media (max-width: 640px) { .nav-search-desktop { display: none !important; } .nav-search-mobile { display: block !important; } .nav-back-mobile { display: flex !important; } .nav-loc-desktop { display: none !important; } }
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(0,0,0,0.97)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", gap: 12, padding: "12px 16px" }}>
          ...
          {/* ✅ LOCATION BUTTON */}
          <button className="nav-loc-desktop" onClick={onLocationClick} style={{
            display: "flex", alignItems: "center", gap: 6,
            background: location ? (zStyle?.bg || "rgba(34,197,94,0.08)") : "rgba(255,255,255,0.04)",
            border: `1px solid ${location ? (zStyle?.border || "rgba(34,197,94,0.25)") : "rgba(255,255,255,0.08)"}`,
          ...
          <div className="nav-search-desktop" style={{ flex: 1, position: "relative" }}>
            ...
            />
            {searchQuery && (
              <button onClick={() => onSearchChange("")} style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "#6b7280", cursor: "pointer", fontSize: 16, padding: 0 }}>×</button>
            )}
          </div>
        ...
        <div className="nav-search-mobile" style={{ display: "none", padding: "0 16px 12px", flexDirection: "column", gap: 8 }}>
          ...
          <button onClick={onLocationClick} style={{ display: "flex", alignItems: "center", gap: 6, background: location ? "rgba(34,197,94,0.08)" : "rgba(255,255,255,0.04)", border: `1px solid ${location ? "rgba(34,197,94,0.25)" : "rgba(255,255,255,0.08)"}`, borderRadius: 50, padding: "8px 16px", cursor: "pointer", width: "fit-content" }}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill={location ? "#22c55e" : "#6b7280"} stroke="none">
              <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
            </svg>
            <span style={{ color: location ? "#22c55e" : "#6b7280", fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600 }}>
              {location ? location.label : "Select delivery location"}
            </span>
          </button>
          {/* Search mobile */}
          <div style={{ position: "relative" }}>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 16 }}>
            {matchedVendors.map(vendor => (
              <VendorCard key={vendor.id} vendor={vendor} onOrderClick={onOrderClick} />
            ))}
          </div>
        </div>
      )}
  ...
  return (
    <>
      <style>{`
        ...
        @media (max-width: 640px) { .nav-back-mobile { display: flex !important; } .nav-search-mobile { display: flex !important; flex-direction: column; } }
      `}</style>
      ...
      <div style={{ minHeight: "100vh", background: "#000" }}>
        ...
        {/* ✅ Location prompt banner — shows when no location selected */}
        {!selectedLocation && (
          <div style={{ background: "rgba(34,197,94,0.06)", borderBottom: "1px solid rgba(34,197,94,0.15)", padding: "10px 20px" }}>
            <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
              <p style={{ color: "#22c55e", fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, margin: 0 }}>
                📍 Set your delivery location so we know where to bring your food!
              </p>
              <button onClick={() => setShowLocationPicker(true)} style={{ background: "#22c55e", color: "#000", border: "none", borderRadius: 50, padding: "7px 18px", fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" }}>
                Set Location →
              </button>
            </div>
          </div>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 16px 100px" }}>
          ...
          {isSearching ? (
            <SearchResults query={searchQuery} onAddToCart={addToCart} onOrderClick={() => setShowLoginPrompt(true)} />
          ) : (
            <>
              {/* Greeting */}
                ...
                >
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16 }}>
                    ...
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                      <button onClick={() => setShowPearlsMenu(true)} style={{ background: "linear-gradient(135deg, #eab308, #ca8a04)", color: "#000", border: "none", borderRadius: 50, padding: "12px 24px", fontFamily: "'DM Sans', sans-serif", fontWeight: 800, fontSize: 14, cursor: "pointer", transition: "all 0.2s", whiteSpace: "nowrap", boxShadow: "0 4px 16px rgba(234,179,8,0.3)" }}
                      ...
                      >Order Now →</button>
                      <Link to="/pearls" style={{ background: "rgba(234,179,8,0.1)", border: "1px solid rgba(234,179,8,0.35)", color: "#eab308", textDecoration: "none", borderRadius: 50, padding: "12px 24px", fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 14, whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 6, transition: "all 0.2s" }}
